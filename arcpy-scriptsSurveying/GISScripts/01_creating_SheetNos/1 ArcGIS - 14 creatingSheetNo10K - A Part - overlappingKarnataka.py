...
outFeatureClass10K = arcpy.CreateFeatureclass_management(outGDB, "sheetNos10K", "POLYGON", None, "DISABLED", "DISABLED", arcpy.SpatialReference(4326))
...
def getSheetNo_50K(lon, lat):
    ...
    suffix = str(int(round(4.5 - 4 * (lat % 1))) + int(round((16 * (lon % 1) - 2)))).zfill(2)
...
lonInc = float(0.05)
latInc = float(0.05)
for lon in np.arange(75.75, 76, lonInc):
    for lat in np.arange(14, 14.25, latInc):
        SWPt = (lon, lat)
